...
void child_cleanup (void *arg)
{
	...
	SPINE_LOG_DEVDBG (("DEBUG: Device[%i] HT[%i] The Device Thread has cleaned up.", poller_details.host_id,
...
void child_cleanup_thread (void *arg)
{
	...
	SPINE_LOG_DEVDBG (
		("DEBUG: Available Threads is %i (%i outstanding)", a_threads_value, set.threads - a_threads_value));
}
...
void child_cleanup_script (void *arg)
{
	...
	SPINE_LOG_DEVDBG (("DEBUG: Available Scripts is %i (%i outstanding)", a_scripts_value,
		MAX_SIMULTANEOUS_SCRIPTS - a_scripts_value));
}
...
void *child (void *arg)
{
	...
	poll_host (
		device_counter, host_id, host_thread, host_threads, host_data_ids, host_time, &host_errors, host_time_double);
...
/*! \fn void poll_host(int device_counter, int host_id, int host_thread, int host_threads, int host_data_ids, char
 **host_time, int *host_errors, double host_time_double) \brief core Spine function that polls a host \param host_id
 *integer value for the host_id from the hosts table in Cacti
 *
 ...
 *
 */
void poll_host (int device_counter, int host_id, int host_thread, int host_threads, int host_data_ids, char *host_time,
	int *host_errors, double host_time_double)
{
	char query1[BUFSIZE];
	...
	*buf_errors = 0;
	...
	if (set.poller_id == 0) {
		...
		snprintf (query2, BIG_BUFSIZE,
			...
			"snmp_priv_passphrase, snmp_priv_protocol, snmp_context, snmp_engine_id, snmp_port, snmp_timeout, "
			"max_oids, "
		...
		if (set.dbonupdate == 0) {
			snprintf (posuffix, BUFSIZE, " ON DUPLICATE KEY UPDATE output=VALUES(output)");
		} else {
			snprintf (posuffix, BUFSIZE, " AS rs ON DUPLICATE KEY UPDATE output=rs.output");
		}
		...
		if (set.total_snmp_ports == 1) {
			snprintf (query1, BUFSIZE,
				...
				" WHERE host_id = %i"
				" AND poller_id=%i %s",
				regex_col, host_id, set.poller_id, limits);
		} else {
			snprintf (query1, BUFSIZE,
				"SELECT SQL_NO_CACHE action, hostname, snmp_community, "
				"snmp_version, snmp_username, snmp_password, "
		...
		snprintf (query6, BUFSIZE,
			...
			" WHERE host_id = %i"
			" AND poller_id = %i",
			set.poller_interval, set.poller_interval, set.poller_interval, set.poller_interval, host_id, set.poller_id);
		...
		if ((result = db_query (&mysql, LOCAL, query2)) != 0) {
			...
			if (num_rows != 1) {
				...
				if (local_cnn != NULL) {
					...
					SPINE_LOG (("WARNING: Device[%i] HT[%i] Trying to close uninitialized local connection.", host_id,
						host_thread));
				}
				...
				if (set.poller_id > 1 && set.mode == REMOTE_ONLINE) {
					if (remote_cnn != NULL) {
						...
						SPINE_LOG (("WARNING: Device[%i] HT[%i] Trying to close uninitialized remote connection.",
							host_id, host_thread));
					}
				}
			...
			if (row) {
				...
				if (row[4] != NULL)
					STRNCOPY (host->snmp_username, row[4]);
				if (row[5] != NULL)
					STRNCOPY (host->snmp_password, row[5]);
				if (row[6] != NULL)
					STRNCOPY (host->snmp_auth_protocol, row[6]);
				if (row[7] != NULL)
					STRNCOPY (host->snmp_priv_passphrase, row[7]);
				if (row[8] != NULL)
					STRNCOPY (host->snmp_priv_protocol, row[8]);
				if (row[9] != NULL)
					STRNCOPY (host->snmp_context, row[9]);
				if (row[10] != NULL)
					STRNCOPY (host->snmp_engine_id, row[10]);

				if (row[11] != NULL)
					host->snmp_port = atoi (row[11]);
				if (row[12] != NULL)
					host->snmp_timeout = atoi (row[12]);
				if (row[13] != NULL)
					host->max_oids = atoi (row[13]);

				if (row[14] != NULL)
					host->availability_method = atoi (row[14]);
				if (row[15] != NULL)
					host->ping_method = atoi (row[15]);
				if (row[16] != NULL)
					host->ping_port = atoi (row[16]);
				if (row[17] != NULL)
					host->ping_timeout = atoi (row[17]);
				if (row[18] != NULL)
					host->ping_retries = atoi (row[18]);

				if (row[19] != NULL)
					host->status = atoi (row[19]);
				if (row[20] != NULL)
					host->status_event_count = atoi (row[20]);

				if (row[21] != NULL)
					STRNCOPY (host->status_fail_date, row[21]);
				if (row[22] != NULL)
					STRNCOPY (host->status_rec_date, row[22]);

				if (row[23] != NULL)
					STRNCOPY (host->status_last_error, row[23]);

				if (row[24] != NULL)
					host->min_time = atof (row[24]);
				if (row[25] != NULL)
					host->max_time = atof (row[25]);
				if (row[26] != NULL)
					host->cur_time = atof (row[26]);
				if (row[27] != NULL)
					host->avg_time = atof (row[27]);
				if (row[28] != NULL)
					host->total_polls = atoi (row[28]);
				if (row[29] != NULL)
					host->failed_polls = atoi (row[29]);
				if (row[30] != NULL)
					host->availability = atof (row[30]);

				if (row[31] != NULL)
					host->snmp_sysUpTimeInstance = atoll (row[31]);
				if (row[32] != NULL)
					db_escape (&mysql, host->snmp_sysDescr, sizeof (host->snmp_sysDescr), row[32]);
				if (row[33] != NULL)
					db_escape (&mysql, host->snmp_sysObjectID, sizeof (host->snmp_sysObjectID), row[33]);
				if (row[34] != NULL)
					db_escape (&mysql, host->snmp_sysContact, sizeof (host->snmp_sysContact), row[34]);
				if (row[35] != NULL)
					db_escape (&mysql, host->snmp_sysName, sizeof (host->snmp_sysName), row[35]);
				if (row[36] != NULL)
					db_escape (&mysql, host->snmp_sysLocation, sizeof (host->snmp_sysLocation), row[36]);

				/* correct max_oid bounds issues */
				if ((host->max_oids == 0) || (host->max_oids > 100)) {
					SPINE_LOG (("Device[%i] HT[%i] WARNING: Max OIDS is out of range with value of '%i'.  Resetting to "
								"default of 5",
				...
				if (((host->snmp_version >= 1) && (host->snmp_version <= 2) && (strlen (host->snmp_community) > 0))
					|| (host->snmp_version == 3)) {
					host->snmp_session = snmp_host_init (host->id, host->hostname, host->snmp_version,
						host->snmp_community, host->snmp_username, host->snmp_password, host->snmp_auth_protocol,
						host->snmp_priv_passphrase, host->snmp_priv_protocol, host->snmp_context, host->snmp_engine_id,
				...
				if ((host->availability_method == AVAIL_SNMP) && (strlen (host->snmp_community) == 0)
					...
					if (is_debug_device (host->id)) {
						SPINE_LOG (("Device[%i] HT[%i] No host availability check possible for '%s'", host->id,
							host_thread, host->hostname));
					} else {
						SPINE_LOG_MEDIUM (("Device[%i] HT[%i] No host availability check possible for '%s'", host->id,
							host_thread, host->hostname));
					}
					...
					if (!ignore_sysinfo) {
						if (host->ignore_host != TRUE) {
							snprintf (update_sql, BIG_BUFSIZE,
								...
								"WHERE id='%i'",
								host->status, host->status_event_count, host->status_fail_date, host->status_rec_date,
								escaped_last_error, host->min_time, host->max_time, host->cur_time, host->avg_time,
								host->total_polls, host->failed_polls, host->availability, host->snmp_sysDescr,
								host->snmp_sysObjectID, host->snmp_sysUpTimeInstance, host->snmp_sysContact,
								host->snmp_sysName, host->snmp_sysLocation, host->id);
						} else {
							snprintf (update_sql, BIG_BUFSIZE,
								"UPDATE host "
								"SET status='%i', status_event_count='%i', status_fail_date=FROM_UNIXTIME(%s),"
								...
								host->status, host->status_event_count, host->status_fail_date, host->status_rec_date,
								escaped_last_error, host->min_time, host->max_time, host->cur_time, host->avg_time,
								host->total_polls, host->failed_polls, host->availability, host->id);
						}
					} else {
						snprintf (update_sql, BIG_BUFSIZE,
							"UPDATE host "
	...
	if (set.ping_only) {
		...
		if (local_cnn != NULL) {
			...
			SPINE_LOG (
				("WARNING: Device[%i] HT[%i] Trying to close uninitialized local connection.", host_id, host_thread));
		}
		...
		if (set.poller_id > 1 && set.mode == REMOTE_ONLINE) {
			if (remote_cnn != NULL) {
				...
				SPINE_LOG (("WARNING: Device[%i] HT[%i] Trying to close uninitialized remote connection.", host_id,
					host_thread));
			}
		}
	...
	if ((!host->ignore_host) && (host_id)) {
		if ((result = db_query (&mysql, LOCAL, query4)) != 0) {
			...
			if (num_rows > 0) {
				if (is_debug_device (host->id)) {
					SPINE_LOG (
						("DEBUG: Device[%i] HT[%i] RECACHE: Processing %i items in the auto reindex cache for '%s'",
							host->id, host_thread, num_rows, host->hostname));
				} else {
					SPINE_LOG_DEBUG (
						("DEBUG: Device[%i] HT[%i] RECACHE: Processing %i items in the auto reindex cache for '%s'",
							host->id, host_thread, num_rows, host->hostname));
				...
				while ((row = mysql_fetch_row (result))) {
					...
					if (row[2] != NULL)
						snprintf (reindex->op, sizeof (reindex->op), "%s", row[2]);

					if (row[3] != NULL)
						snprintf (reindex->assert_value, sizeof (reindex->assert_value), "%s", row[3]);

					if (row[4] != NULL)
						snprintf (reindex->arg1, sizeof (reindex->arg1), "%s", row[4]);
					...
					if (perform_assert) {
						...
						case POLLER_ACTION_SNMP: /* snmp */
							...
							if (!reindex_err) {
								if ((strstr (reindex->arg1, ".1.3.6.1.2.1.1.3.0")
									...
									if (is_debug_device (host->id)) {
										SPINE_LOG (("Device[%i] HT[%i] DQ[%i] Legacy Uptime Result: %s, Is Numeric: %d",
											host->id, host_thread, reindex->data_query_id, poll_result,
											is_numeric (poll_result)));
									} else {
										SPINE_LOG_MEDIUM (
											("Device[%i] HT[%i] DQ[%i] Legacy Uptime Result: %s, Is Numeric: %d",
												host->id, host_thread, reindex->data_query_id, poll_result,
												is_numeric (poll_result)));
									}
									...
									if (is_debug_device (host->id)) {
										SPINE_LOG (
											("Device[%i] HT[%i] DQ[%i] Extended Uptime Result: %s, Is Numeric: %d",
												host->id, host_thread, reindex->data_query_id, poll_result,
												is_numeric (poll_result)));
									} else {
										SPINE_LOG_MEDIUM (
								...
								if (is_debug_device (host->id)) {
									SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE OID: %s, (assert: %s %s output: %s)",
										host->id, host_thread, reindex->data_query_id, reindex->arg1,
										reindex->assert_value, reindex->op, poll_result));
								} else {
									SPINE_LOG_MEDIUM (
										("Device[%i] HT[%i] DQ[%i] RECACHE OID: %s, (assert: %s %s output: %s)",
											host->id, host_thread, reindex->data_query_id, reindex->arg1,
											reindex->assert_value, reindex->op, poll_result));
								}
							} else {
								SPINE_LOG (("WARNING: Device[%i] HT[%i] DQ[%i] Reindex Check FAILED: No SNMP Session.  "
											"If not an SNMP host, don't use Uptime Goes Backwards!",
						...
						case POLLER_ACTION_SCRIPT: /* script (popen) */
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE CMD: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DQ[%i] RECACHE CMD: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							}
						...
						case POLLER_ACTION_PHP_SCRIPT_SERVER: /* script (php script server) */
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE SERVER: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DQ[%i] RECACHE SERVER: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							}
						...
						case POLLER_ACTION_SNMP_COUNT: /* snmp; count items */
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE OID COUNT: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DQ[%i] RECACHE OID COUNT: %s, output: %s",
									host->id, host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							}
						...
						case POLLER_ACTION_SCRIPT_COUNT: /* script (popen); count items by counting line feeds */
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE CMD COUNT: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DQ[%i] RECACHE CMD COUNT: %s, output: %s",
									host->id, host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							}
						...
						case POLLER_ACTION_PHP_SCRIPT_SERVER_COUNT: /* script (php script server); count number of lines
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] HT[%i] DQ[%i] RECACHE SERVER COUNT: %s, output: %s", host->id,
									host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DQ[%i] RECACHE SERVER COUNT: %s, output: %s",
									host->id, host_thread, reindex->data_query_id, reindex->arg1, poll_result));
							}
						...
						if (!reindex_err) {
							...
							if (poll_result == NULL || (IS_UNDEFINED (poll_result))
								|| (STRIMATCH (poll_result, "No Such Instance"))) {
								...
								if (host_thread == 1) {
									snprintf (query3, LRG_BUFSIZE,
										"REPLACE INTO poller_command (poller_id, time, action,command) values (%i, "
										"NOW(), %i, '%i:%i')",
										set.poller_id, POLLER_COMMAND_REINDEX, host->id, reindex->data_query_id);
								...
								if ((!strcmp (reindex->op, "<"))
									...
									if (host_thread == 1) {
										snprintf (query3, LRG_BUFSIZE,
											"REPLACE INTO poller_command (poller_id, time, action, command) VALUES "
											"(%i, NOW(), %i, '%i:%i')",
											set.poller_id, POLLER_COMMAND_REINDEX, host->id, reindex->data_query_id);
							...
							if ((assert_fail) || (!strcmp (reindex->op, ">")) || (!strcmp (reindex->op, "<"))) {
								if (host_thread == 1) {
									...
									snprintf (query3, LRG_BUFSIZE,
										"UPDATE poller_reindex SET assert_value='%s' WHERE host_id='%i' AND "
										"data_query_id='%i' AND arg1='%s'",
										temp_poll_result, host_id, reindex->data_query_id, temp_arg1);
								...
								if ((assert_fail)
									...
									if (is_debug_device (host->id) || set.spine_log_level == 2) {
										...
										SPINE_LOG_MEDIUM (("Device[%i] HT[%i] NOTICE: Spike Kill in Effect for '%s'",
											host_id, host_thread, host->hostname));
									}
								}
							}
				...
				if (is_debug_device (host->id)) {
					...
					SPINE_LOG_HIGH (
						("Device[%i] HT[%i] Device has no information for recache.", host->id, host_thread));
				}
			}
	...
	if (num_rows > 0) {
		...
		while ((row = mysql_fetch_row (result))) {
			...
			if (row[1] != NULL)
				snprintf (poller_items[i].hostname, sizeof (poller_items[i].hostname), "%s", row[1]);
			if (row[2] != NULL)
			...
			if (row[6] != NULL)
				snprintf (poller_items[i].rrd_name, sizeof (poller_items[i].rrd_name), "%s", row[6]);
			if (row[7] != NULL)
				snprintf (poller_items[i].rrd_path, sizeof (poller_items[i].rrd_path), "%s", row[7]);
			if (row[8] != NULL)
				snprintf (poller_items[i].arg1, sizeof (poller_items[i].arg1), "%s", row[8]);
			if (row[9] != NULL)
				snprintf (poller_items[i].arg2, sizeof (poller_items[i].arg2), "%s", row[9]);
			if (row[10] != NULL)
				snprintf (poller_items[i].arg3, sizeof (poller_items[i].arg3), "%s", row[10]);
			...
			if (row[15] != NULL)
				snprintf (
					poller_items[i].snmp_auth_protocol, sizeof (poller_items[i].snmp_auth_protocol), "%s", row[15]);
			if (row[16] != NULL)
				snprintf (
					poller_items[i].snmp_priv_passphrase, sizeof (poller_items[i].snmp_priv_passphrase), "%s", row[16]);
			if (row[17] != NULL)
				snprintf (
					poller_items[i].snmp_priv_protocol, sizeof (poller_items[i].snmp_priv_protocol), "%s", row[17]);
			if (row[18] != NULL)
				snprintf (poller_items[i].snmp_context, sizeof (poller_items[i].snmp_context), "%s", row[18]);
			if (row[19] != NULL)
				snprintf (poller_items[i].snmp_engine_id, sizeof (poller_items[i].snmp_engine_id), "%s", row[19]);

			if (set.has_output_regex && row[20] != NULL)
				snprintf (poller_items[i].output_regex, sizeof (poller_items[i].output_regex), "%s", row[20]);
		...
		if (is_debug_device (host_id)) {
			SPINE_LOG (("Device[%i] HT[%i] NOTE: There are '%i' Polling Items for this Device", host_id, host_thread,
				num_rows));
		} else {
			SPINE_LOG_MEDIUM (("Device[%i] HT[%i] NOTE: There are '%i' Polling Items for this Device", host_id,
				host_thread, num_rows));
		}
		...
		while ((i < num_rows) && (!host->ignore_host)) {
			...
			case POLLER_ACTION_SNMP: /* raw SNMP poll */
				...
				if (k == 0) {
					...
					host->snmp_session = snmp_host_init (host->id, poller_items[i].hostname,
						poller_items[i].snmp_version, poller_items[i].snmp_community, poller_items[i].snmp_username,
						poller_items[i].snmp_password, poller_items[i].snmp_auth_protocol,
						poller_items[i].snmp_priv_passphrase, poller_items[i].snmp_priv_protocol,
						poller_items[i].snmp_context, poller_items[i].snmp_engine_id, poller_items[i].snmp_port,
				...
				if ((last_snmp_port != poller_items[i].snmp_port) || (last_snmp_version != poller_items[i].snmp_version)
					...
					if (num_oids > 0) {
						...
						for (j = 0; j < num_oids; j++) {
							if (host->ignore_host) {
								SPINE_LOG (("Device[%i] HT[%i] DS[%i] WARNING: SNMP timeout detected [%i ms], ignoring "
											"host '%s'",
								...
								buffer_output_errors (error_string, buf_size, buf_errors, host_id, host_thread,
									poller_items[snmp_oids[j].array_position].local_data_id, false);
								errors++;

								if (set.spine_log_level == 2) {
									SPINE_LOG (("WARNING: Invalid Response, Device[%i] HT[%i] DS[%i] SNMP: v%i: %s, "
												"dsname: %s, oid: %s, value: %s",
										host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
								...
								snprintf (temp_result, RESULTS_BUFFER, "%s",
									regex_replace (REGEX_NUMBER, strip_alpha (snmp_oids[j].result)));
								...
								if (!validate_result (snmp_oids[j].result)) {
									...
									if (set.spine_log_level == 2) {
										SPINE_LOG (("WARNING: Invalid Response, Device[%i] HT[%i] DS[%i] SNMP: v%i: "
													"%s, dsname: %s, oid: %s, value: %s",
											host_id, host_thread,
							...
							snprintf (poller_items[snmp_oids[j].array_position].result, RESULTS_BUFFER, "%s",
								snmp_oids[j].result);
							...
							if (is_debug_device (host_id)) {
								SPINE_LOG (
									("Device[%i] HT[%i] DS[%i] TT[%.2f] SNMP: v%i: %s, dsname: %s, oid: %s, value: %s",
										host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
										(float)((thread_end - thread_start) * 1000), host->snmp_version, host->hostname,
										poller_items[snmp_oids[j].array_position].rrd_name,
				...
				if (num_oids >= host->max_oids) {
					...
					for (j = 0; j < num_oids; j++) {
						if (host->ignore_host) {
							SPINE_LOG (
								("Device[%i] HT[%i] DS[%i] WARNING: SNMP timeout detected [%i ms], ignoring host '%s'",
									host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
									host->snmp_timeout, host->hostname));
							SET_UNDEFINED (snmp_oids[j].result);
						} else if (IS_UNDEFINED (snmp_oids[j].result)) {
						...
						if (strlen (poller_items[snmp_oids[j].array_position].output_regex)) {
							snprintf (temp_result, RESULTS_BUFFER, "%s",
								regex_replace (
									poller_items[snmp_oids[j].array_position].output_regex, snmp_oids[j].result));
							snprintf (snmp_oids[j].result, RESULTS_BUFFER, "%s", temp_result);
						}
			...
			case POLLER_ACTION_SCRIPT: /* execute script file */
				...
				if (strlen (poller_items[i].arg1) == 0) {
					SPINE_LOG (("WARNING: Device[%i] HT[%i] DS[%i] empty script command, skipping", host_id,
						host_thread, poller_items[i].local_data_id));
					SET_UNDEFINED (poller_items[i].result);
					break;
				}
				...
				if (IS_UNDEFINED (poll_result)) {
					SET_UNDEFINED (poller_items[i].result);
					buffer_output_errors (
						error_string, buf_size, buf_errors, host_id, host_thread, poller_items[i].local_data_id, false);
					errors++;

					if (set.spine_log_level == 2) {
						SPINE_LOG (("WARNING: Invalid Response, Device[%i] HT[%i] DS[%i] SCRIPT: %s, output: %s",
							host_id, host_thread, poller_items[i].local_data_id, poller_items[i].arg1,
					...
					/* remove double or single quotes from string */
					snprintf (
						temp_result, RESULTS_BUFFER, "%s", regex_replace (REGEX_NUMBER, strip_alpha (poll_result)));
					...
					if (!validate_result (poller_items[i].result)) {
						buffer_output_errors (error_string, buf_size, buf_errors, host_id, host_thread,
							poller_items[i].local_data_id, false);
						errors++;
				...
				if (is_debug_device (host_id)) {
					SPINE_LOG (("Device[%i] HT[%i] DS[%i] TT[%.2f] SCRIPT: %s, output: %s", host_id, host_thread,
						poller_items[i].local_data_id, (float)((thread_end - thread_start) * 1000),
						poller_items[i].arg1, poller_items[i].result));
				} else {
					SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DS[%i] TT[%.2f] SCRIPT: %s, output: %s", host_id, host_thread,
						poller_items[i].local_data_id, (float)((thread_end - thread_start) * 1000),
						poller_items[i].arg1, poller_items[i].result));
				}
			...
			case POLLER_ACTION_PHP_SCRIPT_SERVER: /* execute script server */
				/* Reject empty script commands that could cause unexpected behavior */
				if (strlen (poller_items[i].arg1) == 0) {
					SPINE_LOG (("WARNING: Device[%i] HT[%i] DS[%i] empty script server command, skipping", host_id,
						host_thread, poller_items[i].local_data_id));
					SET_UNDEFINED (poller_items[i].result);
					break;
				}
				...
				if (is_debug_device (host_id)) {
					SPINE_LOG (("Device[%i] HT[%i] DS[%i] TT[%.2f] SS[%i] SERVER: %s, output: %s", host_id, host_thread,
						poller_items[i].local_data_id, (float)((thread_end - thread_start) * 1000), php_process,
						poller_items[i].arg1, poller_items[i].result));
				} else {
					SPINE_LOG_MEDIUM (("Device[%i] HT[%i] DS[%i] TT[%.2f] SS[%i] SERVER: %s, output: %s", host_id,
						host_thread, poller_items[i].local_data_id, (float)((thread_end - thread_start) * 1000),
						php_process, poller_items[i].arg1, poller_items[i].result));
				}
			...
			default: /* unknown action, generate error */
				SPINE_LOG (("Device[%i] HT[%i] DS[%i] ERROR: Unknown Poller Action: %s", host_id, host_thread,
					poller_items[i].local_data_id, poller_items[i].arg1));
		...
		if (num_oids > 0) {
			...
			for (j = 0; j < num_oids; j++) {
				if (host->ignore_host) {
					...
					if (set.spine_log_level == 2) {
						SPINE_LOG (("WARNING: Invalid Response, Device[%i] HT[%i] DS[%i] SNMP: v%i: %s, dsname: %s, "
									"oid: %s, value: %s",
							host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
					...
					if (!validate_result (snmp_oids[j].result)) {
						...
						if (set.spine_log_level == 2) {
							SPINE_LOG (("WARNING: Invalid Response, Device[%i] HT[%i] DS[%i] SNMP: v%i: %s, dsname: "
										"%s, oid: %s, value: %s",
								host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
								host->snmp_version, host->hostname, poller_items[snmp_oids[j].array_position].rrd_name,
								poller_items[snmp_oids[j].array_position].arg1, snmp_oids[j].result));
				...
				if (is_debug_device (host_id)) {
					SPINE_LOG (("Device[%i] HT[%i] DS[%i] TT[%.2f] SNMP: v%i: %s, dsname: %s, oid: %s, value: %s",
						host_id, host_thread, poller_items[snmp_oids[j].array_position].local_data_id,
						(float)((thread_end - thread_start) * 1000), host->snmp_version, host->hostname,
						poller_items[snmp_oids[j].array_position].rrd_name,
						poller_items[snmp_oids[j].array_position].arg1,
					...
					SPINE_LOG_MEDIUM (
						("Device[%i] HT[%i] DS[%i] TT[%.2f] SNMP: v%i: %s, dsname: %s, oid: %s, value: %s", host_id,
		...
		while (i < rows_processed) {
			...
			snprintf (result_string, RESULTS_BUFFER + SMALL_BUFSIZE, " (%i, '%s', FROM_UNIXTIME(%s), '%s')",
				poller_items[i].local_data_id, escaped_rrd_name, host_time, escaped_result);
			...
			if ((out_buffer + result_length) >= MAX_MYSQL_BUF_SIZE) {
				...
				if (set.boost_redirect && set.boost_enabled && query12) {
					/* append the suffix */
	...
	if (details[device_counter]->threads_complete == details[device_counter]->host_threads) {
		...
		query1[0] = '\0';
		snprintf (query1, BUFSIZE, "UPDATE host SET polling_time = %.3f - %.3f WHERE id = %i", poll_time,
			host_time_double, host_id);
	...
	if (errors > 0) {
		...
		snprintf (error_query, error_query_len,
			"INSERT INTO host_errors (host_id, poller_id, errors, local_data_ids)"
			" VALUES(%i, %i, %i, '%s')"
	...
	if (set.poller_id > 1 && set.mode == REMOTE_ONLINE) {
		if (remote_cnn != NULL) {
			...
			SPINE_LOG (
				("WARNING: Device[%i] HT[%i] Trying to close uninitialized remote connection.", host_id, host_thread));
		}
	}
	...
	if (is_debug_device (host_id)) {
		SPINE_LOG (("DEBUG: Device[%i] HT[%i] DEBUG: HOST COMPLETE: About to Exit Device Polling Thread Function",
			host_id, host_thread));
	} else {
		SPINE_LOG_DEBUG (("DEBUG: Device[%i] HT[%i] DEBUG: HOST COMPLETE: About to Exit Device Polling Thread Function",
			host_id, host_thread));
	}
	...
	char *error_string, int *buf_size, int *buf_errors, int device_id, int thread_id, int local_data_id, bool flush)
{
	...
	if (flush && *buf_errors > 0) {
		SPINE_LOG (("WARNING: Invalid Response(s), Errors[%i] Device[%i] Thread[%i] DS[%s]", *buf_errors, device_id,
...
void get_system_information (host_t *host, MYSQL *mysql, int system)
{
	...
	if (set.mibs || system) {
		...
		SPINE_LOG_DEVDBG (
			("DEVDBG: Device[%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.1.0'); [complete]", host->id));
		...
		SPINE_LOG_DEVDBG (
			("DEVDBG: Device[%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.2.0'); [complete]", host->id));
		...
		SPINE_LOG_DEVDBG (
			("DEVDGB: Device[%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.3.0'); [complete]", host->id));
		...
		if (poll_result && is_numeric (poll_result)) {
			...
			SPINE_LOG_DEVDBG (
				("DEVDBG: Device[%d] poll_result = snmp_get(host, '.1.3.6.1.6.3.10.2.1.3.0');", host->id));
			poll_result = snmp_get_base (host, ".1.3.6.1.6.3.10.2.1.3.0", false);
			SPINE_LOG_DEVDBG (
				("DEVDGB: Device[%d] poll_result = snmp_get(host, '.1.3.6.1.6.3.10.2.1.3.0'); [complete]", host->id));
		...
		SPINE_LOG_DEVDBG (
			("DEVDBG: Device [%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.4.0'); [complete]", host->id));
		...
		SPINE_LOG_DEVDBG (
			("DEVDBG: Device [%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.5.0'); [complete]", host->id));
		...
		SPINE_LOG_DEVDBG (
			("DEVDBG: Device [%d] poll_result = snmp_get(host, '.1.3.6.1.2.1.1.6.0'); [complete]", host->id));
...
/* compensate for back slashes in arguments */
	...
	while (++retries < (set.script_timeout * 15)) {
		...
		if (sem_err == 0) {
			...
			if (is_debug_device (current_host->id)) {
				SPINE_LOG (
					("DEBUG: Device[%i]: Pausing as unable to obtain a script execution lock", current_host->id));
			} else {
				SPINE_LOG_DEVDBG (
					("DEBUG: Device[%i]: Pausing as unable to obtain a script execution lock", current_host->id));
			}
			...
			if (is_debug_device (current_host->id)) {
				SPINE_LOG (("DEBUG: Device[%i]: Pausing as error %d whilst obtaining a script execution lock",
					current_host->id, sem_err));
			} else {
				SPINE_LOG_DEVDBG (("DEBUG: Device[%i]: Pausing as error %d whilst obtaining a script execution lock",
					current_host->id, sem_err));
			}
		}
		usleep (10000);
	...
	if (sem_err) {
		...
		if (access (executable, X_OK | F_OK) != -1) {
			...
			cmd_fd = fileno (fd);
			if (is_debug_device (current_host->id)) {
				SPINE_LOG (("DEBUG: Device[%i] DEBUG: The POPEN returned the following File Descriptor %i",
					current_host->id, cmd_fd));
			} else {
				SPINE_LOG_DEBUG (("DEBUG: Device[%i] DEBUG: The POPEN returned the following File Descriptor %i",
					current_host->id, cmd_fd));
			}
#else
			cmd_fd = nft_popen (proc_command, "r");
			if (is_debug_device (current_host->id)) {
				SPINE_LOG (("DEBUG: Device[%i] DEBUG: The NIFTY POPEN returned the following File Descriptor %i",
					current_host->id, cmd_fd));
			} else {
				SPINE_LOG_DEBUG (("DEBUG: Device[%i] DEBUG: The NIFTY POPEN returned the following File Descriptor %i",
					current_host->id, cmd_fd));
			}
#endif
			...
			retry:
				...
				case -1:
					switch (errno) {
					case EBADF:
						SPINE_LOG (("Device[%i] ERROR: One or more of the file descriptor sets specified a file "
					...
					case EINTR:
						...
						break;
					case EINVAL:
						SPINE_LOG (("Device[%i] ERROR: Possible invalid timeout specified in select() statement.",
							current_host->id));
						SET_UNDEFINED (result_string);
#ifdef USING_TPOPEN
						close_fd = FALSE;
				...
				default:
					...
					if (bytes_read > 0) {
						...
						if (STRIMATCH (type, "DS")) {
							SPINE_LOG (("Device[%i] DS[%i] ERROR: Empty result [%s]: '%s'", current_host->id, id,
								current_host->hostname, command));
						} else {
							SPINE_LOG (("Device[%i] DQ[%i] ERROR: Empty result [%s]: '%s'", current_host->id, id,
								current_host->hostname, command));
						}
						SET_UNDEFINED (result_string);
					}
...
/* close pipe */
				...
				SPINE_LOG (("Device[%i] ERROR: Problem executing POPEN [%s]: '%s'", current_host->id,
					current_host->hostname, command));
				SET_UNDEFINED (result_string);
			}
		} else {
			SPINE_LOG (("Device[%i] ERROR: Problem executing POPEN.  File '%s' does not exist or is not executable.",
				current_host->id, command));
			SET_UNDEFINED (result_string);
		}
