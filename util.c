...
static bool putsetting (MYSQL *psql, int mode, const char *mysetting, const char *myvalue)
{
	...
	if (set.dbonupdate == 0) {
		snprintf (qstring, sizeof (qstring),
...
void read_config_options (void)
{
	...
	SPINE_LOG_DEBUG (("DEBUG: The PHP Script Server is %sRequired", set.php_required ? "" : "Not "));
...
void poller_push_data_to_main (void)
{
	...
	if (strlen (set.host_id_list)) {
		snprintf (query, MEGA_BUFSIZE,
			"SELECT SQL_NO_CACHE id, snmp_sysDescr, snmp_sysObjectID, "
			...
			"WHERE poller_id = %d "
			"AND id IN (%s)",
			set.poller_id, set.host_id_list);
	} else {
		snprintf (query, MEGA_BUFSIZE,
			"SELECT SQL_NO_CACHE id, snmp_sysDescr, snmp_sysObjectID, "
			"snmp_sysUpTimeInstance, snmp_sysContact, snmp_sysName, snmp_sysLocation, "
	...
	snprintf (prefix, BUFSIZE,
		"INSERT INTO host (id, snmp_sysDescr, snmp_sysObjectID, "
		"snmp_sysUpTimeInstance, snmp_sysContact, snmp_sysName, snmp_sysLocation, "
		"status, status_event_count, status_fail_date, status_rec_date, "
		"status_last_error, min_time, max_time, cur_time, avg_time, polling_time, "
	...
	if (strlen (set.host_id_list)) {
		snprintf (query, MEGA_BUFSIZE,
			"SELECT SQL_NO_CACHE local_data_id, host_id, rrd_name, rrd_step, rrd_next_step "
			"FROM poller_item "
			"WHERE poller_id = %d "
			"AND host_id IN (%s)",
			set.poller_id, set.host_id_list);
	...
	snprintf (
		prefix, BUFSIZE, "INSERT INTO poller_item (local_data_id, host_id, rrd_name, rrd_step, rrd_next_step) VALUES ");
	...
	if ((fp = fopen (file, "rb")) == NULL) {
		...
		while (!feof (fp)) {
			...
			if (chars != NULL && !feof (fp) && *buff != '#' && *buff != ' ' && *buff != '\n') {
				...
				if (STRIMATCH (p1, "RDB_Host"))
					STRNCOPY (set.rdb_host, p2);
				else if (STRIMATCH (p1, "RDB_Database"))
					STRNCOPY (set.rdb_db, p2);
				else if (STRIMATCH (p1, "RDB_User"))
					STRNCOPY (set.rdb_user, p2);
				else if (STRIMATCH (p1, "RDB_Pass"))
					STRNCOPY (set.rdb_pass, p2);
				else if (STRIMATCH (p1, "RDB_Port"))
					set.rdb_port = atoi (p2);
				else if (STRIMATCH (p1, "RDB_UseSSL"))
					set.rdb_ssl = atoi (p2);
				else if (STRIMATCH (p1, "RDB_SSL_Key"))
					STRNCOPY (set.rdb_ssl_key, p2);
				else if (STRIMATCH (p1, "RDB_SSL_Cert"))
					STRNCOPY (set.rdb_ssl_cert, p2);
				else if (STRIMATCH (p1, "RDB_SSL_CA"))
					STRNCOPY (set.rdb_ssl_ca, p2);
				else if (STRIMATCH (p1, "DB_Host"))
					STRNCOPY (set.db_host, p2);
				else if (STRIMATCH (p1, "DB_Database"))
					STRNCOPY (set.db_db, p2);
				else if (STRIMATCH (p1, "DB_User"))
					STRNCOPY (set.db_user, p2);
				else if (STRIMATCH (p1, "DB_Pass"))
					STRNCOPY (set.db_pass, p2);
				else if (STRIMATCH (p1, "DB_Port"))
					set.db_port = atoi (p2);
				else if (STRIMATCH (p1, "DB_UseSSL"))
					set.db_ssl = atoi (p2);
				else if (STRIMATCH (p1, "DB_SSL_Key"))
					STRNCOPY (set.db_ssl_key, p2);
				else if (STRIMATCH (p1, "DB_SSL_Cert"))
					STRNCOPY (set.db_ssl_cert, p2);
				else if (STRIMATCH (p1, "DB_SSL_CA"))
					STRNCOPY (set.db_ssl_ca, p2);
				...
				} else if (STRIMATCH (p1, "SNMP_Clientaddr"))
					STRNCOPY (set.snmp_clientaddr, p2);
...
int spine_log (const char *format, ...)
{
	...
	snprintf (logprefix, LOGSIZE, "SPINE: Poller[%i] PID[%i] PT[%lu] ", set.poller_id, getpid (),
