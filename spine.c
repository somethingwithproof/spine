...
void drop_root (uid_t server_uid, gid_t server_gid)
{
	...
	if (setregid (server_gid, server_gid) == -1 || setreuid (server_uid, server_uid) == -1) {
		SPINE_LOG_HIGH (("setregid/setreuid to uid=%d/gid=%d failed.", server_uid, server_gid));
		exit (1);
	}

	SPINE_LOG_LOW (("running as uid(%d)/gid(%d) euid(%d)/egid(%d) with cap_net_raw=eip.", getuid (), getgid (),
		geteuid (), getegid ()));
}
#endif /* HAVE_LCAP */
...
/* we attempt to support scripts better in cygwin */
	...
	if ((HOSTID_DEFINED (set.start_host_id) != HOSTID_DEFINED (set.end_host_id)) && (!strlen (set.host_id_list))) {
	...
	/* obtain the list of hosts to poll */
	{
		...
		qp += snprintf (qp, remaining,
			"SELECT SQL_NO_CACHE id, device_threads, picount, picount/device_threads AS tppi FROM host AS h LEFT JOIN "
			"(SELECT host_id, COUNT(*) AS picount FROM poller_item GROUP BY host_id) AS pi ON h.id = pi.host_id");
		...
		remaining = MEGA_BUFSIZE - (qp - querybuf);
		snprintf (qp, remaining, " ORDER BY picount DESC");
	}
	...
	if (!set.ping_only) {
		snprintf (querybuf, BIG_BUFSIZE,
			"INSERT INTO poller_time (poller_id, pid, start_time, end_time) VALUES (%i, %i, NOW(), '0000-00-00 "
			"00:00:00')",
	...
	SPINE_LOG_HIGH (("DEBUG: Initial Value of Available Threads is %i (%i outstanding)", a_threads_value,
	...
	while (canexit == FALSE && device_counter < num_rows) {
		...
		if (!set.ping_only) {
			if (set.active_profiles != 1) {
				snprintf (querybuf, BIG_BUFSIZE,
					"SELECT SQL_NO_CACHE COUNT(local_data_id) FROM poller_item WHERE host_id=%i AND rrd_next_step <=0",
					host_id);
			} else {
				snprintf (querybuf, BIG_BUFSIZE,
					"SELECT SQL_NO_CACHE COUNT(local_data_id) FROM poller_item WHERE host_id=%i", host_id);
			}
		...
		if (device_threads > 1) {
			if (current_thread == 1) {
				if (set.active_profiles != 1) {
					snprintf (querybuf, BIG_BUFSIZE,
						"SELECT SQL_NO_CACHE CEIL(COUNT(local_data_id)/%i) FROM poller_item WHERE host_id=%i AND "
						"rrd_next_step <=0",
						device_threads, host_id);
				} else {
					snprintf (querybuf, BIG_BUFSIZE,
						"SELECT SQL_NO_CACHE CEIL(COUNT(local_data_id)/%i) FROM poller_item WHERE host_id=%i",
						device_threads, host_id);
				}
		...
		while (TRUE) {
			...
			if (sem_err == 0) {
				...
				SPINE_LOG_DEVDBG (("WARNING: Device[%i] HT[%i] would have deadlocked acquiring Available Thread Lock",
		...
		while (!spine_timeout) {
			...
			if (sem_err == 0) {
				...
				SPINE_LOG_DEVDBG (
					("WARNING: Device[%i] HT[%i] would have deadlocked acquiring Thread Initialization Lock", host_id,
						current_thread));
			} else if (sem_err == EAGAIN) {
				// Keep trying
			} else {
				SPINE_LOG_DEVDBG (
					("WARNING: Device[%i] HT[%i] errored with %d while acquiring Thread Initialization Lock", host_id,
						current_thread, sem_err));
			}
		...
		thread_retry:
			...
			if (thread_status == 0) {
				SPINE_LOG_DEBUG (("DEBUG: Device[%i] Valid Thread to be Created (%ld)", poller_details->host_id,
					(unsigned long int)threads[device_counter]));
				...
				spine_sem_getvalue (&available_threads, &a_threads_value);
				SPINE_LOG_HIGH (("DEBUG: Device[%i] Available Threads is %i (%i outstanding)", poller_details->host_id,
					a_threads_value, set.threads - a_threads_value));
				...
				SPINE_LOG_DEVDBG (("DEBUG: DTS: device = %d, host_id = %d, host_thread = %d,"
					...
					device_counter - 1, poller_details->host_id, poller_details->host_thread,
					poller_details->host_threads, poller_details->host_data_ids, poller_details->complete));
	...
	if (!set.ping_only) {
		...
		for (threads_count = 0; threads_count < num_rows; threads_count++) {
			...
			if (det != NULL) { // && !det->complete) {
				SPINE_LOG_HIGH (
					("INFO: Device[%i] Thread %scomplete and %d to %d sources", det->host_id, det->complete ? "" : "in",
						det->host_data_ids * (det->host_thread - 1), det->host_data_ids * (det->host_thread)));

				SPINE_LOG_DEVDBG (("DEBUG: DTF: device = %d, host_id = %d, host_thread = %d,"
								   " host_threads = %d, host_data_ids = %d, complete = %d",
					threads_count, det->host_id, det->host_thread, det->host_threads, det->host_data_ids,
					det->complete));
			}
		}
	...
	if (!set.ping_only) {
		...
		snprintf (querybuf, BIG_BUFSIZE, "UPDATE poller_time SET end_time=NOW() WHERE poller_id=%i AND pid=%i",
			set.poller_id, getpid ());
...
static void display_help (int only_version)
{
	...
	static const char *const helptext[] = {
		...
		"  -f/--first=X       Start polling with host id X", "  -l/--last=X        End polling with host id X",
		"  -H/--hostlist=X    Poll the list of host ids, separated by comma's",
		"  -p/--poller=X      Set the poller id to X", "  -t/--threads=X     Override the database threads setting.",
		...
		"                     The default is 'online'.", "  -R/--readonly      Spine will not write output to the DB",
		...
		"In their absence, all hosts are processed.", "", "Without the --conf parameter, spine searches in order:",
		...
		"database, but they can be overridden with the --option=S:V", "parameter.", "",
