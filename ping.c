...
int ping_host (host_t *host, ping_t *ping)
{
	...
	if ((host->availability_method == AVAIL_SNMP_AND_PING) || (host->availability_method == AVAIL_PING)
		...
		if (!strstr (host->hostname, "localhost")) {
			if (get_address_type (host) == 1) {
				...
				snprintf (ping->ping_response, SMALL_BUFSIZE,
					"PING: Device is Unknown or is IPV6.  Please use the SNMP ping options only.");
	...
	/* snmp test */
	if ((host->availability_method == AVAIL_SNMP) || (host->availability_method == AVAIL_SNMP_GET_SYSDESC)
		|| (host->availability_method == AVAIL_SNMP_GET_NEXT) || (host->availability_method == AVAIL_SNMP_AND_PING)
		|| (host->availability_method == AVAIL_SNMP_OR_PING)) {
			...
			if ((host->availability_method != AVAIL_SNMP_OR_PING)
				...
				if (snmp_result == HOST_UP) {
					if (is_debug_device (host->id)) {
						SPINE_LOG (("Device[%i] INFO: SNMP Device Alive, Time:%.4f ms", host->id,
							snmp_end_time - snmp_start_time));
					} else {
						SPINE_LOG_MEDIUM (("Device[%i] INFO: SNMP Device Alive, Time:%.4f ms", host->id,
							snmp_end_time - snmp_start_time));
					}
				} else {
					if (is_debug_device (host->id)) {
						SPINE_LOG (("Device[%i] INFO: SNMP Device Down, Time:%.4f ms", host->id,
							snmp_end_time - snmp_start_time));
					} else {
						SPINE_LOG_MEDIUM (("Device[%i] INFO: SNMP Device Down, Time:%.4f ms", host->id,
							snmp_end_time - snmp_start_time));
					}
				}
			}
	...
	if ((strlen (host->hostname) != 0) && (icmp_socket != -1)) {
		...
		if (init_sockaddr (&fromname, host->hostname, 7)) {
			...
			while (1) {
				...
				if (is_debug_device (host->id)) {
					SPINE_LOG (("Device[%i] DEBUG: Attempting to ping %s, seq %d (Retry %d of %d)", host->id,
						host->hostname, icmp->icmp_seq, retry_count, host->ping_retries));
				} else {
					SPINE_LOG_DEBUG (("DEBUG: Device[%i] Attempting to ping %s, seq %d (Retry %d of %d)", host->id,
						host->hostname, icmp->icmp_seq, retry_count, host->ping_retries));
				}
				...
				return_code
					= sendto (icmp_socket, packet, packet_len, 0, (struct sockaddr *)&fromname, sizeof (fromname));
			...
			keep_listening:
				...
				if (icmp_socket >= FD_SETSIZE) {
					SPINE_LOG (
						("ERROR: Device[%i] ICMP socket %d exceeds FD_SETSIZE %d", host->id, icmp_socket, FD_SETSIZE));
					snprintf (ping->ping_status, 50, "down");
				...
				if (total_time < host_timeout) {
					...
					return_code = recvfrom (
						icmp_socket, socket_reply, BUFSIZE, MSG_WAITALL, (struct sockaddr *)&recvname, &fromlen);
#else
					return_code = recvfrom (
						icmp_socket, socket_reply, BUFSIZE, MSG_PEEK, (struct sockaddr *)&recvname, &fromlen);
#endif
						...
						if (fromname.sin_addr.s_addr == recvname.sin_addr.s_addr) {
							if (pkt->icmp_type == ICMP_ECHOREPLY) {
								if (is_debug_device (host->id)) {
									...
									SPINE_LOG_MEDIUM (("Device[%i] INFO: ICMP Device Alive, Try Count:%i, Time:%.4f ms",
										host->id, retry_count + 1, (total_time)));
								}
	...
	if ((strlen (host->hostname) != 0) && (udp_socket != -1)) {
		...
		if (init_sockaddr (&servername, host->hostname, host->ping_port)) {
			...
			wait_more:
				...
				if (return_code > 0) {
					if (FD_ISSET (udp_socket, &socket_fds)) {
						...
						if (return_code == -1
							...
							if (is_debug_device (host->id)) {
								SPINE_LOG (("Device[%i] INFO: UDP Device Alive, Try Count:%i, Time:%.4f ms", host->id,
									retry_count + 1, (total_time)));
							} else {
								SPINE_LOG_MEDIUM (("Device[%i] INFO: UDP Device Alive, Try Count:%i, Time:%.4f ms",
									host->id, retry_count + 1, (total_time)));
							}
				...
				if (is_debug_device (host->id)) {
					SPINE_LOG (("Device[%i] DEBUG: UDP Timeout, Try Count:%i, Time:%.4f ms", host->id, retry_count + 1,
						(total_time)));
				} else {
					SPINE_LOG_DEBUG (("DEBUG: Device[%i] UDP Timeout, Try Count:%i, Time:%.4f ms", host->id,
						retry_count + 1, (total_time)));
				}
	...
	if ((strlen (host->hostname) != 0) && (tcp_socket != -1)) {
		...
		if (init_sockaddr (&servername, host->hostname, host->ping_port)) {
			...
			while (1) {
				...
				if ((return_code == -1 && errno == ECONNREFUSED && host->ping_method == PING_TCP_CLOSED)
					|| return_code == 0) {
					if (is_debug_device (host->id)) {
						SPINE_LOG (("Device[%i] INFO: TCP Device Alive, Try Count:%i, Time:%.4f ms", host->id,
							retry_count + 1, (total_time)));
					} else {
						SPINE_LOG_MEDIUM (("Device[%i] INFO: TCP Device Alive, Try Count:%i, Time:%.4f ms", host->id,
							retry_count + 1, (total_time)));
					}
...
int get_address_type (host_t *host)
{
	...
	for (res = res_list; res != NULL; res = res->ai_next) {
		...
		SPINE_LOG_HIGH (("Device[%d] IPv%d address %s (%s)", host->id, res->ai_family == PF_INET6 ? 6 : 4, addrstr,
			res->ai_canonname));
...
int init_sockaddr (struct sockaddr_in *name, const char *hostname, unsigned short int port)
{
	...
	while (true) {
		...
		if (rv == 0) {
			...
			case EAI_AGAIN:
				if (retry_count < 3) {
					SPINE_LOG (("WARNING: EAGAIN received resolving after 3 retryies for host %s (%s)", hostname,
						gai_strerror (rv)));
					if (hostinfo != NULL) {
					...
					SPINE_LOG (
						("WARNING: Error resolving after 3 retryies for host %s (%s)", hostname, gai_strerror (rv)));
					if (hostinfo != NULL) {
						freeaddrinfo (hostinfo);
					}
					return false;
			...
			case EAI_FAIL:
				SPINE_LOG (
					("WARNING: DNS Server reported permanent error for host %s (%s)", hostname, gai_strerror (rv)));
				if (hostinfo != NULL) {
					freeaddrinfo (hostinfo);
				}
				return false;
		...
		if (tokens == 1) {
			if (strlen (token) && token[0] == '[') {
				...
				if (strncasecmp (token, "TCP", 3) == 0) {
					...
					SPINE_LOG_DEBUG (
						("DEBUG: get_namebyhost(%s) - No matching method for 3 chars: %s", hostname, token));
				...
				if (strncasecmp (token, "TCP6", 4) == 0) {
					...
					SPINE_LOG_DEBUG (
						("DEBUG: get_namebyhost(%s) - No matching method for 4 chars: %s", hostname, token));

					// assume we have had a method
					tokens++;
				}
			} else {
				SPINE_LOG_DEBUG (("DEBUG: get_hostbyname(%s) - No matching method for %li chars: %s", hostname,
					strlen (token), token));
...
void update_host_status (int status, host_t *host, ping_t *ping, int availability_method)
{
	...
	if (status == HOST_DOWN) {
		...
		/* average time */
		host->avg_time = (((host->total_polls - 1 - host->failed_polls) * host->avg_time) + ping_time)
			/ (host->total_polls - host->failed_polls);
	...
	if (issue_log_message) {
		if (host->status == HOST_DOWN) {
			SPINE_LOG (("Device[%i] Hostname[%s] ERROR: HOST EVENT: Device is DOWN Message: %s", host->id,
				host->hostname, host->status_last_error));
		} else {
			SPINE_LOG (("Device[%i] Hostname[%s] NOTICE: HOST EVENT: Device Returned from DOWN State", host->id,
				host->hostname));
		}
	}
}
