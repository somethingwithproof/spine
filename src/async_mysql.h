...
/* Seed the owner-thread id for the g_active_mysql invariant check.
 * Must be called from the libuv main-loop thread BEFORE any worker
 * threads are created, so the subsequent assertion in mark/unmark
 * cannot race its own first store. Idempotent on the same thread;
 * die()s on a second call from a different thread. */
