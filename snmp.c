 ...
 *
 */
void *snmp_host_init (int host_id, char *hostname, int snmp_version, char *snmp_community, char *snmp_username,
	char *snmp_password, char *snmp_auth_protocol, char *snmp_priv_passphrase, char *snmp_priv_protocol,
	char *snmp_context, char *snmp_engine_id, int snmp_port, int snmp_timeout)
{
...
/* Prevent update of the snmpapp.conf file */
	...
	SPINE_LOG_HIGH (("Device[%i] INFO: SNMP Device '%s' has a timeout of %ld (%d), with %d retries", host_id,
		hostnameport, session.timeout, snmp_timeout, session.retries));
	...
	if ((snmp_version == 2) || (snmp_version == 1)) {
		...
		/* set the authentication protocol */
		{
				...
				if (Apsz) {
					...
					if (session.securityAuthProto == NULL) {
						session.securityAuthProto
							= snmp_duplicate_objid (SNMP_DEFAULT_AUTH_PROTO, SNMP_DEFAULT_AUTH_PROTOLEN);
						session.securityAuthProtoLen = SNMP_DEFAULT_AUTH_PROTOLEN;
					}

					if (generate_Ku (session.securityAuthProto, session.securityAuthProtoLen, (u_char *)Apsz,
							strlen (Apsz), session.securityAuthKey, &session.securityAuthKeyLen)
						!= SNMPERR_SUCCESS) {
						SPINE_LOG (
							("SNMP: Device[%i] Error generating SNMPv3 Ku from authentication passphrase.", host_id));
						free (session.peername);
						free (session.securityAuthProto);
						free (session.securityPrivProto);
						free (Apsz);
				...
				if (Xpsz) {
					...
					if (session.securityPrivProto == NULL) {
						session.securityPrivProto
							= snmp_duplicate_objid (SNMP_DEFAULT_PRIV_PROTO, SNMP_DEFAULT_PRIV_PROTOLEN);
						session.securityPrivProtoLen = SNMP_DEFAULT_PRIV_PROTOLEN;
					}

					if (generate_Ku (session.securityAuthProto, session.securityAuthProtoLen, (u_char *)Xpsz,
							strlen (Xpsz), session.securityPrivKey, &session.securityPrivKeyLen)
						!= SNMPERR_SUCCESS) {
			...
			SPINE_LOG_MEDIUM (("Device[%i] SNMPv3 Using AuthProto: %s, PrivProto: %s", host_id, snmp_auth_protocol,
				snmp_priv_protocol));
		} /* end auth/priv block */
	}
...
char *snmp_get_base (host_t *current_host, const char *snmp_oid, bool should_fail)
{
	...
	if (current_host->ignore_host) {
		SPINE_LOG_HIGH (
			("WARNING: Skipped oid '%s' for Device[%i] as host ignore flag is active", snmp_oid, current_host->id));
	...
	if (current_host->snmp_session != NULL) {
		...
		if (status == STAT_ERROR && current_host->snmp_session != NULL) {
			...
			sess_liberr = liberr;
			SPINE_LOG_DEBUG (("Device[%i] DEBUG: SNMP session error for oid '%s': %s (liberr=%d)", current_host->id,
				snmp_oid, errstr ? errstr : "unknown", liberr));
			SNMP_FREE (errstr);
		}
		...
		if (status == STAT_DESCRIP_ERROR) {
			...
			if (response == NULL) {
				...
				SET_UNDEFINED (result_string);
				status = STAT_ERROR;
			} else if (response->errstat == SNMP_ERR_NOERROR && response->variables != NULL
				...
				if (vars->type == SNMP_NOSUCHOBJECT) {
					if (!strstr (snmp_oid, ".1.3.6.1.2.1.1.1.0") && !strstr (snmp_oid, ".1.3.6.1.2.1.1.3.0")) {
						SET_UNDEFINED (result_string);
						status = STAT_ERROR;
						SPINE_LOG_DEBUG (("DEBUG: OID '%s' for Device[%i], SNMP_NOSUCHOBJECT not sysDesc or sysUptime",
							snmp_oid, current_host->id));
					} else {
						SPINE_LOG_HIGH (("DEBUG: OID '%s' for Device[%i], SNMP_NOSUCHOBJECT for sysDesc or sysUptime",
							snmp_oid, current_host->id));
					...
					if (!strstr (snmp_oid, ".1.3.6.1.6.3.10.2.1.3.0")) {
						SPINE_LOG_HIGH (("WARNING: No such Instance for oid '%s' for Device[%i] with Status[%d]",
							snmp_oid, current_host->id, status));
					} else {
						SPINE_LOG_DEBUG (("NOTE: Legacy SNMP agent found!  No per second Uptime oid found '%s' for "
										  "Device[%i] with Status[%d]",
							snmp_oid, current_host->id, status));
					}
				} else if (vars->type == SNMP_ENDOFMIBVIEW) {
					SET_UNDEFINED (result_string);
					status = STAT_ERROR;
					SPINE_LOG_HIGH (("ERROR: End of Mib for oid '%s' for Device[%i] with Status[%d]", snmp_oid,
				...
				SPINE_LOG_HIGH (("ERROR: Failed to get oid '%s' for Device[%i] with Response[%ld]", snmp_oid,
					current_host->id, response->errstat));
			}
			...
			if (vars->type == SNMP_NOSUCHOBJECT) {
				SET_UNDEFINED (result_string);
				status = STAT_ERROR;
				SPINE_LOG_HIGH (("ERROR: No such Object for oid '%s' for Device[%i] with Status[%d]", snmp_oid,
					current_host->id, status));
			} else if (vars->type == SNMP_NOSUCHINSTANCE) {
				SET_UNDEFINED (result_string);
				status = STAT_ERROR;
				...
				// We will ignore the new OID error
				if (!strstr (snmp_oid, ".1.3.6.1.6.3.10.2.1.3.0")) {
					SPINE_LOG_HIGH (("WARNING: No such Instance for oid '%s' for Device[%i] with Status[%d]", snmp_oid,
						current_host->id, status));
				} else {
					SPINE_LOG_DEBUG (
						("NOTE: Per second level uptime oid missing oid '%s' for Device[%i] with Status[%d]", snmp_oid,
							current_host->id, status));
				}
			} else if (vars->type == SNMP_ENDOFMIBVIEW) {
				SET_UNDEFINED (result_string);
				...
				SET_UNDEFINED (result_string);
				status = STAT_ERROR;
				SPINE_LOG_HIGH (("ERROR: Unknown error getting oid '%s' for Device[%i] with Status[%d]", snmp_oid,
					current_host->id, status));
			}
		} else if (status == STAT_TIMEOUT) {
			SPINE_LOG_HIGH (
				("ERROR: Timeout getting oid '%s' for Device[%i] with Status[%d]", snmp_oid, current_host->id, status));
		} else {
...
#endif
				SPINE_LOG_MEDIUM (
					("WARNING: Device[%i] USM notInTimeWindow for oid '%s' -- engine time drift (recoverable)",
...
#endif
				SPINE_LOG_HIGH (("ERROR: Device[%i] USM unknownEngineID for oid '%s'", current_host->id, snmp_oid));
				break;
#endif
...
#endif
				SPINE_LOG_HIGH (("ERROR: Device[%i] USM unknownSecurityName for oid '%s'", current_host->id, snmp_oid));
				break;
#endif
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM authenticationFailure for oid '%s'", current_host->id, snmp_oid));
				break;
#endif
...
#endif
				SPINE_LOG_HIGH (("ERROR: Device[%i] USM decryptionError for oid '%s'", current_host->id, snmp_oid));
				break;
#endif
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM unsupportedSecurityLevel for oid '%s'", current_host->id, snmp_oid));
				break;
#endif
			default:
			...
			if (response == NULL) {
				...
				if (response->errstat == SNMP_ERR_NOERROR) {
					...
					if (vars != NULL) {
						...
						snprint_asciistring (
							result_string, RESULTS_BUFFER, (unsigned char *)temp_result, strlen (temp_result));
					} else {
						SET_UNDEFINED (result_string);
						status = STAT_ERROR;
					}
...
#endif
				SPINE_LOG_MEDIUM (
					("WARNING: Device[%i] USM notInTimeWindow for getnext oid '%s' -- engine time drift (recoverable)",
						current_host->id, snmp_oid));
				/* treat as transient, do not mark host down */
				status = STAT_SUCCESS;
				SET_UNDEFINED (result_string);
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM unknownEngineID for getnext oid '%s'", current_host->id, snmp_oid));
				break;
#endif
#if defined(SNMPERR_UNKNOWN_USER_NAME) || defined(SNMPERR_USM_UNKNOWNSECURITYNAME)
#if defined(SNMPERR_UNKNOWN_USER_NAME)
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM unknownSecurityName for getnext oid '%s'", current_host->id, snmp_oid));
				break;
#endif
#if defined(SNMPERR_AUTHENTICATION_FAILURE) || defined(SNMPERR_USM_AUTHENTICATIONFAILURE)
#if defined(SNMPERR_AUTHENTICATION_FAILURE)
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM authenticationFailure for getnext oid '%s'", current_host->id, snmp_oid));
				break;
#endif
#if defined(SNMPERR_DECRYPTION_ERR) || defined(SNMPERR_USM_DECRYPTIONERROR)
#if defined(SNMPERR_DECRYPTION_ERR)
...
#endif
				SPINE_LOG_HIGH (
					("ERROR: Device[%i] USM decryptionError for getnext oid '%s'", current_host->id, snmp_oid));
				break;
#endif
#if defined(SNMPERR_UNSUPPORTED_SEC_LEVEL) || defined(SNMPERR_USM_UNSUPPORTEDSECURITYLEVEL)
#if defined(SNMPERR_UNSUPPORTED_SEC_LEVEL)
...
#endif
				SPINE_LOG_HIGH ((
					"ERROR: Device[%i] USM unsupportedSecurityLevel for getnext oid '%s'", current_host->id, snmp_oid));
				break;
#endif
			default:
				SPINE_LOG_HIGH (("ERROR: Unknown error getting oid '%s' for Device[%i] with Status[%d] Errno[%d]",
...
/*! \fn void snmp_snprint_value(char *obuf, size_t buf_len, const oid *objid, size_t objidlen, struct variable_list
 **variable)
 *
...
void snmp_get_multi (host_t *current_host, target_t *poller_items, snmp_oids_t *snmp_oids, int num_oids)
{
	...
	for (i = 0; i < num_oids; i++) {
		...
		if (!snmp_parse_oid (snmp_oids[i].oid, namep->name, &namep->name_len)) {
			SPINE_LOG (("Device[%i] DS[%i] ERROR: Problems parsing Multi SNMP OID! (oid: %s), Set MAX_OIDS to 1 for "
						"this host to isolate bad OID",
				current_host->id, poller_items[snmp_oids[i].array_position].local_data_id, snmp_oids[i].oid));
		...
		if (response == NULL) {
			...
			if (response->errstat == SNMP_ERR_NOERROR) {
				...
				if (response->errindex != 0) {
					...
					SPINE_LOG_HIGH (("Device[%i] WARNING: snmp_get_multi PDU error (errstat=%ld, errindex=0), all OIDs "
									 "set undefined",
